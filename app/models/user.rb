class User < ApplicationRecord
  ...
  def follow!(other_user, matching_score)
    @relationship = active_relationships.new(
      followed_id: other_user.id, total_score: matching_score[0],
      positive: matching_score[1], faithful: matching_score[2],
      cooperative: matching_score[3], mental: matching_score[4],
      curious: matching_score[5], background: matching_score[6])
    @relationship.save!
    @relationship
  end
